...
export function CreationStudio() {
    ...
    try {
      if (selectedFormat === "image") {
        const response = await fetch("/api/generate-image", {
          ...
          body: JSON.stringify({ style: selectedStyle, customPrompt: promptText })
        });
      ...
      const seed = Date.now();
      const media = getMediaVariant(selectedStyle, selectedFormat, 0);
      const item: CommunityItem = {
        id: `generated-${seed}`,
        author: "나",
        type: selectedFormat,
  ...
  return (
    <div className="panel-section">
      ...
      <label className="control-group" htmlFor="creation-prompt">
        <span className="control-label">창작 프롬프트</span>
        <textarea
